...
class Department(BaseModel, BaseAdminModel, ImageModel):
    pass
    
class Division(BaseModel, BaseAdminModel, ImageModel):
    department = models.ForeignKey(Department, on_delete=models.CASCADE, related_name="%(app_label)s_%(class)s_department")

class Project(BaseModel, ImageModel):
    project_leader = models.ManyToManyField(User, verbose_name="Project Leaders", related_name="%(app_label)s_%(class)s_projct_leads")

class Subproject(BaseModel, ImageModel):
    lead = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name="Subproject Leader", related_name="%(app_label)s_%(class)s_lead")

class Task(BaseModel, ImageModel):
